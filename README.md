...
Renders a self-contained `report.html` + a `badge.svg` from a bundled example —
zero `claude` calls, zero money. The HTML is an interactive, offline dashboard
(no CDN/JS deps): a verdict hero, per-arm cards, a metric selector that drives
...
work in real time (cell grid + console + usage ticker), and a **results** view that
embeds the interactive report + badge. Try it with zero spend via the **"Run the
...
skill-ab badge -s /tmp/skill_ab_results/summary.json # a verified/regressed/inconclusive badge
```
...
|---|---|
| `demo` | offline, free example report + badge |
...
| `badge` | emit an SVG + Markdown verdict badge from `summary.json` |
| `ci` | run + gate with a policy exit code (see `.github/workflows/skill-ab.yml`) |

The badge is **self-policing**: it only reads "verified" when the 95% CI excludes 0
*and* the run is trustworthy (≥2 clustered tasks, no contamination) — grey otherwise.
...
badge's headline becomes the A-vs-B comparison.
